...
UPLOAD_FOLDER = os.path.join(os.path.dirname(
    os.path.abspath(__name__)), 'uploads')
...
def criar_aluno():
    ...
    if not all([nome, nome_mae, data_nascimento, curso, turma, turno]):
        return jsonify({"erro": "Todos os campos de texto são obrigatórios."}), 400

    # Gera uma matrícula única mais curta (13 caracteres) e em maiúsculo
    matricula = str(uuid.uuid4())[:13].upper()

    # Salva o arquivo com a matricula para evitar nomes duplicados
...
# Aceitando POST como fallback caso haja erro com XMLHttpRequest
@app.route('/api/alunos/<int:aluno_id>', methods=['PUT', 'POST'])
def atualizar_aluno(aluno_id):
    # Validando se o aluno existe
    aluno_existente = database.obter_aluno(aluno_id)
    ...
    try:
        atualizado = database.atualizar_aluno(
            aluno_id, nome, matricula, nome_mae, data_nascimento, curso, turma, turno, nome_arquivo)
    ...
    try:
        ...
        if aluno.get('foto'):
            caminho_foto = os.path.join(
                app.config['UPLOAD_FOLDER'], aluno['foto'])
...
# Aceitando POST como fallback
@app.route('/api/alunos/<int:aluno_id>/impresso', methods=['PATCH', 'POST'])
def marcar_impresso(aluno_id):
    ...
    if not dados and request.form.get('impresso') is not None:
        impresso = request.form.get(
            'impresso') == 'true' or request.form.get('impresso') == '1'
    elif not dados and request.args.get('impresso') is not None:
        impresso = request.args.get(
            'impresso') == 'true' or request.args.get('impresso') == '1'
    else:
